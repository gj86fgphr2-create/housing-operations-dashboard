...
def build_customer_wechat_trend():
    """Aggregate the todo workbench's current WeChat-customer records without publishing identifiers."""
    source=Path(os.environ.get("TODO_CUSTOMERS_FILE","/opt/yuxiaor-automation/notifications/state/operations-customers.json"))
    empty={"generatedAt":"","sourceRecordCount":0,"windowRecordCount":0,"startDate":"","endDate":"","dailyRows":[],"weeks":[],"months":[],"channelTotals":{"xiaohongshu":0,"xianyu":0,"other":0}}
    if not source.is_file(): return empty
    loaded=json.loads(source.read_text(encoding="utf-8"))
    records=loaded.get("records",[]) if isinstance(loaded,dict) else []
    normalized=[]
    for record in records:
        date_text=str(record.get("addedDate") or "").strip()
        if not re.fullmatch(r"\d{4}-\d{2}-\d{2}",date_text): continue
        try: day=datetime.strptime(date_text,"%Y-%m-%d").date()
        except ValueError: continue
        source_text=str(record.get("channelSource") or "").strip()
        channel="xiaohongshu" if source_text=="小红书" else "xianyu" if source_text=="咸鱼" else "other"
        normalized.append((day,channel))
    if not normalized: return empty
    end_date=max(day for day,_ in normalized); start_date=end_date-timedelta(days=29)
    counts=defaultdict(lambda:{"xiaohongshu":0,"xianyu":0,"other":0})
    for day,channel in normalized:
        if start_date<=day<=end_date: counts[day][channel]+=1
    rows=[]
    for offset in range(30):
        day=end_date-timedelta(days=offset); channel_counts=counts[day]
        week_suffix="W1" if day.day<=7 else "W2" if day.day<=14 else "W3" if day.day<=21 else "W4" if day.day<=28 else "WE"
        rows.append({"date":day.isoformat(),"monthKey":day.strftime("%Y-%m"),"weekKey":f"{day.month}{week_suffix}","total":sum(channel_counts.values()),**channel_counts})
    def summarize(key):
        grouped={}
        for row in rows:
            group_key=row[key]
            if group_key not in grouped: grouped[group_key]={"key":group_key,"startDate":row["date"],"endDate":row["date"],"total":0,"xiaohongshu":0,"xianyu":0,"other":0}
            group=grouped[group_key]
            group["startDate"]=min(group["startDate"],row["date"]); group["endDate"]=max(group["endDate"],row["date"])
            for field in ("total","xiaohongshu","xianyu","other"): group[field]+=row[field]
...
required += ['data-legacy-occupancy-overview-hidden="true"','[data-legacy-occupancy-overview-hidden="true"]{display:none!important}']
...
required += ['data-desktop-module="customer"','data-desktop-menu="customer"','data-mobile-module="customer"','data-mobile-menu="customer"','id="customer-data"','data-dashboard-view="customer-data"','id="customer-data-updated"','id="customer-daily-table"','id="customer-funnel-table"','id="customer-wechat-trend-chart"','id="customer-wechat-trend-table"','function renderCustomerData()','function renderCustomerWechatTrend()','"customerData"','"wechatTrend"']
...
required += ['data-label="预定数量" class="\' + newStatus + \'"','currencyInteger(metrics.newSignAverageRent)+\'）\'']
...
required += ['id="checkout-reason-detail-table"','class="checkout-reason-table"','过去30天退租原因明细','总数包含其他原因','<th>日期</th><th>实际退租</th><th>到期</th><th>违约</th><th>续租</th><th>其他</th>','reasonTotals.actual','reasonTotals.other','30天合计']
...
required += ['最近30个自然日合同净变化','展示最近30个自然日，空白日期按0计','contract-net-month-band','contract-net-week-band','const newSignRows=','const checkoutByDate=','summarizeBands(','newSignCount-checkoutCount','最新日期在左']
required += ['.contract-daily-trend-panel .line-chart{width:1344px;height:430px}','const height = 430;']
required += ['.contract-net-week-detail{fill:#516074;font-size:13.5px','contract-net-week-detail.compact{font-size:11.25px}','contract-net-month-main.negative,.contract-net-week-main.negative,.chart-value.negative','month.netCount<0?\' negative\':\'\'','week.netCount<0?\' negative\':\'\'','row.total<0?\' negative\':\'\'']
...
required += ['折线图最新日期在左','const chartRows=account.rows.slice(-14).reverse()','latestRow=chartRows[0]']
...
missing_required=[x for x in required if x not in rendered]
if missing_required: raise RuntimeError(f"Full dashboard style validation failed; missing={missing_required}")
...
if not trends_section or 'xhs-traffic-trend-chart' not in trends_section.group(0) or 'xhs-traffic-decline-grid' not in trends_section.group(0) or trends_section.group(0).count('<article class="panel">') != 2: raise RuntimeError("XHS reading and lead trends must use two panels")
if 'id="xhs-daily-reading-chart"' not in trends_section.group(0) or 'id="xhs-reading-decline-grid"' not in trends_section.group(0) or trends_section.group(0).index('id="xhs-daily-reading-chart"') > trends_section.group(0).index('id="xhs-traffic-trend-chart"'): raise RuntimeError("XHS reading trend panel must lead the trends view")
...
xhs_notes_section=re.search(r'<section class="section" id="xhs-notes".*?</section>\s*<section class="section" id="xhs-note-published"',rendered,re.S)
xhs_leads_section=re.search(r'<section class="section" id="xhs-leads".*?</section>\s*<section class="section" id="xhs-lead-details"',rendered,re.S)
if not xhs_notes_section or 'xhs-note-count-table' in xhs_notes_section.group(0): raise RuntimeError("XHS note goal table must be removed from notes view")
if 'xhs-daily-reading-chart' in xhs_notes_section.group(0) or 'xhs-reading-decline-grid' in xhs_notes_section.group(0): raise RuntimeError("XHS reading trend panel must be removed from notes view")
if not xhs_leads_section or 'xhs-note-count-table' not in xhs_leads_section.group(0) or xhs_leads_section.group(0).index('xhs-note-count-table') < xhs_leads_section.group(0).index('xhs-lead-copied-table'): raise RuntimeError("XHS note goal table must appear below lead goals")
if rendered.count('<a href="#xhs-leads" data-dashboard-view="xhs-leads">目标</a>') != 2: raise RuntimeError("XHS target menu must exist on desktop and mobile")
xhs_desktop_menu=re.search(r'<nav class="desktop-secondary-nav" data-desktop-menu="xiaohongshu".*?</nav>',rendered,re.S)
xhs_mobile_menu=re.search(r'<nav class="mobile-menu mobile-secondary-nav xhs-menu" data-mobile-menu="xiaohongshu".*?</nav>',rendered,re.S)
if not xhs_desktop_menu or xhs_desktop_menu.group(0).index('data-dashboard-view="xhs-leads"') < xhs_desktop_menu.group(0).index('data-dashboard-view="xhs-account"') or xhs_desktop_menu.group(0).index('data-dashboard-view="xhs-leads"') > xhs_desktop_menu.group(0).index('data-dashboard-view="xhs-notes"'): raise RuntimeError("XHS target menu must follow account on desktop")
if not xhs_mobile_menu or xhs_mobile_menu.group(0).index('data-dashboard-view="xhs-leads"') < xhs_mobile_menu.group(0).index('data-dashboard-view="xhs-account"') or xhs_mobile_menu.group(0).index('data-dashboard-view="xhs-leads"') > xhs_mobile_menu.group(0).index('data-dashboard-view="xhs-notes"'): raise RuntimeError("XHS target menu must follow account on mobile")
...
wechat_trend=payload["customerData"].get("wechatTrend",{})
wechat_rows=wechat_trend.get("dailyRows",[])
if wechat_rows:
    wechat_fields=("xiaohongshu","xianyu","other")
    dates=[datetime.strptime(row["date"],"%Y-%m-%d").date() for row in wechat_rows]
    if len(wechat_rows)!=30 or len(set(dates))!=30 or any(dates[index]-dates[index+1]!=timedelta(days=1) for index in range(29)): raise RuntimeError("WeChat customer trend must contain 30 descending natural days")
    if any(row["total"]!=sum(row[field] for field in wechat_fields) for row in wechat_rows): raise RuntimeError("WeChat customer trend channel totals do not reconcile")
...
desktop_yuxiaor=re.search(r'<nav class="desktop-secondary-nav" data-desktop-menu="yuxiaor".*?</nav>',rendered,re.S)
mobile_yuxiaor=re.search(r'<nav class="mobile-menu mobile-secondary-nav" data-mobile-menu="yuxiaor".*?</nav>',rendered,re.S)
if not desktop_yuxiaor or 'occupancy-ziyin' in desktop_yuxiaor.group(0): raise RuntimeError("Ziyin occupancy menu must stay hidden on desktop")
if mobile_yuxiaor and 'occupancy-ziyin' in mobile_yuxiaor.group(0): raise RuntimeError("Ziyin occupancy menu must stay hidden on mobile")
